...
const PriceAuction = (props: {
  ...
  const quoteMintName = props.attributes?.quoteMintInfoExtended?.name || "CustomToken"
            ...
            <label className="action-field">
              ...
              <Input
                ...
                suffix={props.attributes.quoteMintInfoExtended? props.attributes.quoteMintInfoExtended.symbol
                    : props.attributes.quoteMintAddress == WRAPPED_SOL_MINT.toBase58()? "SOL": "Jelly"}
                onChange={info =>
                  props.setAttributes({
                    ...props.attributes,
          ...
          <label className="action-field">
            ...
            <Input
              ...
              placeholder={`Tick size in ${props.attributes.quoteMintInfoExtended? props.attributes.quoteMintInfoExtended.symbol
                : props.attributes.quoteMintAddress == WRAPPED_SOL_MINT.toBase58()? "SOL": "CustomToken"}`}
              prefix=""
              suffix={props.attributes.quoteMintInfoExtended? props.attributes.quoteMintInfoExtended.symbol
                : props.attributes.quoteMintAddress == WRAPPED_SOL_MINT.toBase58()? "SOL": "CustomToken"}
              onChange={info =>
...
const Congrats = (props: {
  ...
  const newTweetURL = () => {
    const params = {
      text: "I've created a new NFT auction on Metaplex-V1, check it out!",
      url: `${
        window.location.origin
      }/#/auction/${props.auction?.auction.toString()}`,
      hashtags: 'NFT,Crypto,CEC',
      // via: "Metaplex",
      related: 'NFT,Solana',
    };
